...
class SystemSetting extends Form
{
    ...
    public function form()
    {
        ...
        $this->tab('安全设置', function () {
            $this->number('throttle_order_search', '订单查询限流（次/分钟）')
                ->default(10)
                ->min(1)
                ->max(100)
                ->help('每分钟允许查询订单的次数，默认10次');
            $this->number('throttle_email_search', '邮箱查询限流（次/分钟）')
                ->default(10)
                ->min(1)
                ->max(100)
                ->help('每分钟允许通过邮箱查询的次数，默认10次');
            $this->number('throttle_browser_search', '浏览器查询限流（次/分钟）')
                ->default(20)
                ->min(1)
                ->max(100)
                ->help('每分钟允许通过浏览器查询的次数，默认20次');
            $this->number('security_log_keep_days', '安全日志保留天数')
                ->default(30)
                ->min(1)
                ->max(365)
                ->help('安全日志保留天数，超过将自动清理');
        });
        $this->confirm(
