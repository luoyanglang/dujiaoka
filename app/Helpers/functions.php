    ...
    function replace_mail_tpl($mailtpl = [], $data = [])
    {
        ...
        if ($data) {
            foreach ($data as $key => $val) {
                // XSS 防护：对用户输入的数据进行转义（除了 ord_info 已经是 HTML 格式）
                $safeVal = ($key === 'ord_info') ? $val : htmlspecialchars($val, ENT_QUOTES, 'UTF-8');
                $title = str_replace('{' . $key . '}', $safeVal, isset($title) ? $title : $mailtpl['tpl_name']);
                $content = str_replace('{' . $key . '}', $safeVal, isset($content) ? $content : $mailtpl['tpl_content']);
            }
